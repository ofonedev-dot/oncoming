...
function openModes() {
  if (modes.open || garage.open) return;
  modes.open = true;
  player.speed = 0;
...
function updatePlayer(dt) {
  ...
  const boosting = !garage.open && input.boost && player.boost > 0 && player.crashT <= 0;
...
function primaryWaypoint() {
  if (garage.open || modes.open) return null;
...
function updateHud() {
  ...
  hud.controls.innerHTML = `<div><kbd>WASD</kbd> / <kbd>Arrows</kbd> drive · <kbd>Shift</kbd> boost · <kbd>Space</kbd> brake · garage bay to swap parts${pad}</div>
